...
# --- КАТАЛОГ ---  ✅ ИСПРАВЛЕНО: вынесен из функции admin()
...
# --- НАЖАЛ КУПИТЬ — создаём счёт ---  ✅ ИСПРАВЛЕНО: вынесен из функции admin()
...
# --- ПРОВЕРКА ОПЛАТЫ ---  ✅ ИСПРАВЛЕНО: вынесен из функции admin()
...
async def check_payment(c: types.CallbackQuery):
    ...
    # ⌛️ ИСТЁК  ✅ ИСПРАВЛЕНО: убран elif внутри другого elif
    elif status == "expired":
        kb = InlineKeyboardMarkup(inline_keyboard=[
...
# ✅ ИСПРАВЛЕНО: main() вынесен на верхний уровень
async def main():
